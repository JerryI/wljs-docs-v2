...
<Callout>
A ready to-go example is in [this repository](https://github.com/WLJSTeam/wljs-plugin-example-3)

Clone it to `<Documents>/WLJS Notebooks/Extensions/` using:

```bash
git clone https://github.com/WLJSTeam/wljs-plugin-example-3
```
...
In this tutorial we will add new cell type, which implements basic Wolfram expression evaluation and returns the result in `InputForm`. Our new cell type will start from a simple
...
```json title="package.json"
{
    ...
    "repository": {
        "type": "git",
        "url": "https://github.com/WLJSTeam/wljs-plugin-example-3"
    }
}
```
...
This one checks if kernel is ready. The evaluation itself is abstracted from kernels. Some of cell types do not involve *evaluation kernel* at all: [Javascript](./../../../Cell-types/Javascript), [Shell](./../../../Cell-types/Misc), [Mermaid](./../../../Cell-types/Misc). That is the reason, we have those checks on the level of evaluators. 
        ...
        With[{result = (ToExpression[ t["Data"], InputForm, Hold]) // ReleaseHold },
            ...
            ,   
                With[{string = ToString[result, InputForm]},
                    EventFire[Internal`Kernel`RemoteEvent[ t["Hash"] ], "Result", <|"Data" -> string, "Display"->"codemirror", "Meta"->Sequence["Hash"->hash] |> ];
                ]
            ];
        ];
    ] 
...
Now restart WLJS Notebook and enjoy pure `InputForm` with no syntax sugar:

<div className="invertColor">

![](./imgs/Screenshot-2025-02-12-at-19.25.50-62f0274caa243e81daf804ab2031b7f4.png)

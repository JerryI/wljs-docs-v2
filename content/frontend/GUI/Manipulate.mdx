...
An unsupported change does not normally make the expression invalid: the widget falls back to replacing the complete result. The exception is an option documented as *ignored* below; such an option is not updated while the optimized expression remains active.
...
| --- | --- |
| Coordinates/data of `Line` | Directly inside `Graphics` or `Graphics3D`; changing line data is also supported inside a 3D `GraphicsComplex` |
| Coordinates/data of `Point`, `Triangle`, and `Arrow` | Directly inside `Graphics` or `Graphics3D`, but not inside `GraphicsComplex` |
| Coordinates/data of `Polygon` | Directly inside 2D `Graphics`; in 3D, put the polygon in `GraphicsComplex` |
...
This distinction is important. A changing 2D polygon should remain a direct primitive:
...
Avoid `Graphics3D[Polygon[vertices[t]]]`: direct changing `Polygon` data is not a 3D JIT target. Conversely, putting changing polygon data in a 2D `GraphicsComplex` does not help; dynamic `GraphicsComplex` optimization is implemented for `Graphics3D`.

#### `GraphicsComplex` constraints

For changing 3D vertex or index data, keep the number and arrangement of primitive expressions, the `GraphicsComplex` argument count, and its option order fixed. Vertex arrays and nested `Line` or `Polygon` index arrays can be updated independently. The transpiler recognizes these forms:
...
Use rectangular numeric arrays. The array contents—and, for supported nested `Line` and `Polygon` payloads, their indices—may change, but keep the surrounding primitive list and wrappers structurally compatible. Other option combinations or orders are not JIT-transpiled. Inside a 3D `GraphicsComplex`, changing `Line` and `Polygon` data is supported; changing `Point`, `Triangle`, `Arrow`, shape primitives, or direct color directives is not. Use `VertexColors` for changing per-vertex colors.

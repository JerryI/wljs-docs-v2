...
constructs a group container for frontend expressions (or symbols).

Normally one can append objects on existing plots using [FrontSubmit](../Frontend-IO/FrontSubmit), this group container allows to undo those action by disposing the created instances (even if they are nested). It does also support batch operations.
...
and dispose created instanced and revert any side effects

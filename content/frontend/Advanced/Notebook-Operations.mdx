---
title: Notebooks
description: Control WLJS notebooks programmatically with Wolfram Language functions for creating, reading, writing, evaluating, and managing cells and notebooks.
---
...
Use [Cells](../Cells-and-Notebook/Cells) to list all cell objects in the notebook
...
Programmatic control over the notebook structure allows you to reuse cells as storage for your data. For example, `Cell` representation automatically picks up a cell type if applicable. If you want to call a Python script from the session, where should you store `.py` expressions? Why not store them in __another input cell__ and leave the name in the first line?
...
<Callout type="tip">Enable <small>Expert Mode</small> from the settings menu to see hidden cells in notebook templates</Callout>
...
<Callout type="info">Check our blog to find more about mini apps</Callout>
...
In the case of lost context, i.e. `MessageDialog` is called by a timer, button click - it will find a window to show automatically, unless you explicitly provide `"Notebook"` as an option:
...
```wolfram
Button["Message", Then[ChoiceDialogAsync["Proceed with operation?"], Function[choise,
  Print[choise];
]]]
```
...
System dialogs are no longer associated with notebooks:

```wolfram
SetTimeout[Then[SystemDialogInputAsync["FileSave"], Print], 1000];
```

<Callout type="warning">
    `SystemDialogInputAsync` will not work if it WLJS is running on a server (Docker Image)
</Callout>

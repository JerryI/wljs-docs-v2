...
The number of cell subtypes roughly corresponds to the number of available [cell types](./../Cell%20types/Misc) in WLJS Notebook.
...
Each visible window of WLJS Notebook can be retrieved from the context using [CurrentWindow](./../Frontend%20IO/CurrentWindow)
...
- `WindowObj` is required for certain actions such as script execution on the window done by [FrontSubmit](./../Frontend%20IO/FrontSubmit) and others
...
Notebooks exported as [mini apps](./../Share/Mini-apps) use the very last cell's output as the main window. To take advantage of better customization, we use a [WLX](./../Cell%20types/WLX) cell as the output instead of a plain Wolfram language cell:

...
We will use [event system](./Overview) as a main way of piping data from Javascript to Wolfram side.
...
core.MicStart = async () => {
    if (isRunning) return;
    isRunning = true;   
    // Some browsers need this on a user gesture
...
Module[{line = {}, fft={}},
      EventHandler["audio", Function[data,
        line = Transpose[{Range[data//Length], data}];
        fft = Drop[Transpose[{Range[128]//N, Log@Take[Fourier[data], 128]//Abs}],1];
      ]];
    
      {Graphics[
        {Blue, Line[line // Offload]},
        PlotRange -> {{0,512}, {127-50, 127+50}},
        TransitionDuration->30,
        Axes->True, AspectRatio->1/2
      ],
...
<LazyAutoplayVideo url={"/mic-2.mp4"}/>
...
function resizeCanvas(cssW = CSS_WIDTH, cssH = CSS_HEIGHT) {
    dpr = Math.max(1, Math.min(window.devicePixelRatio || 1, 3)); // cap optional
...
function convertRange(value, [a1, a2], [b1, b2]) {
...
function drawParticles() {
    ...
    for (let i = 0; i < liveCount; i++) {
      ...
      const a = p.baseAlpha * alphaForAge(p.age);
      if (a <= 0) continue;

      const radius = p.size / Math.max(p.age, 0.001);
...
function animate() {
    ...
    const currentDpr = Math.max(1, Math.min(window.devicePixelRatio || 1, 3));
    if (currentDpr !== dpr) resizeCanvas();
...
core.Spark = async (args, env) => {
    ...
    const { minX, minY, maxX, maxY } = getMinMax(raw);
    ...
    data = raw.map(([x, y]) => {
      const mx = convertRange(x, [minX, maxX], [x0, x1]);
      const my = convertRange(y, [maxY, minY], [y0, y1]); // invert Y
...
<LazyAutoplayVideo url={"/spark-1.mp4"}/>

...
- Upon evaluation, can expand the reference back to the Wolfram Expression
...
The example below is also valid if **your definitions match** on the Wolfram Kernel and frontend. *But we generally recommend to separate display function and public symbols*
...
If you want to provide **the same expression** for evaluation, convert the output object to `StandardForm` and supply `ViewBox` with it:

```wolfram
SomeSymbol /: MakeBoxes[s_SomeSymbol, StandardForm] := With[{
  display = CreateFrontEndObject[s]
}, {
  out = MakeBoxes[display, StandardForm]
},
  ViewBox[out, display]
]
```

Or if you want to compress only the part of expression and manually construct the rest:

```wolfram
SomeSymbol /: MakeBoxes[SomeSymbol[arg1_, arg2_], StandardForm] := With[{
  fe1 = CreateFrontEndObject[arg1]
}, {
  uid = ToString[fe1[[1]], InputForm],
  out2 = ToString[arg2, InputForm],
  name = ToString[SomeSymbol, InputForm]
},
  ViewBox[RowBow[{name, "[", "FrontEndRef", "[", uid , "]", ",", out2, "]"}], displaySymbol[fe1, arg2]]
]
```


<Callout>
`ViewBox` also accepts `RowBox` as the first argument allowing you manually construct "invisible" part of the expression
</Callout>

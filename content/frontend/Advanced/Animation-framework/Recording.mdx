...
or use `ffmpeg` (recommended):

```bash
ffmpeg -y -stats_period 60 -pattern_type glob -framerate 60 -i "output_dir/*.png" -c:v prores_ks -profile:v 4 -pix_fmt yuv444p10le prores4444.mov
```

...
### From Animate
You can wrap [Animate](../GUI/Animate), [AnimatePlot](../Plotting-Functions/AnimatePlot), [AnimateParametericPlot](../Plotting-Functions/AnimateParametericPlot) expressions with `AnimatedImage` to rasterize them into the series of images:
...
```wolfram
animation = AnimatedImage[Rasterize[Style[#, 40]] & /@ Range[0, 9]];
Export["anim.gif", animation]
```

```wolfram
animation = AnimatedImage[Rasterize[Style[#, 40]] & /@ Range[0, 9]];
ExportByteArray[animation, "GIF"]
```

```wolfram
animation = AnimatedImage[Rasterize[Style[#, 40]] & /@ Range[0, 9]];
ExportString[animation, {"Base64", "GIF"}]
```

...
---
asynchronously evaluates (aka [FrontSubmit](../Frontend%20IO/FrontSubmit)) and fetches the resulting expression back to the Wolfram Kernel from the frontend (browser)
...
## Options
### `"Window"`
specifies a window socket, to which an expression will be sent. Use [CurrentWindow](../Frontend%20IO/CurrentWindow) to fetch a window object from the evaluation context explicitly.
...
Using an extension [FrontInstanceReference](../Frontend%20IO/FrontInstanceReference), one can execute an expression in the context of a specified container and fetch the result back

```wolfram
FrontFetchAsync[expr_, m_FrontInstanceReference, opts___] _Promise
```

See examples on [FrontSubmit](../Frontend%20IO/FrontSubmit)

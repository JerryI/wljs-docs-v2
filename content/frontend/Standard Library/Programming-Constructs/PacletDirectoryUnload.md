---
title: PacletDirectoryUnload
---

`PacletDirectoryUnload[dir]` makes paclets in dir no longer visible in the current session.

- `PacletDirectoryUnload[{dir1, dir2, …}]` makes paclets in all the diri no longer visible in the current session.

## Examples

```wolfram
PacletDirectoryUnload["~/MyPaclets"]
```

*Please visit the official [Wolfram Language Reference](https://reference.wolfram.com/language/ref/PacletDirectoryUnload.html) for more details.*
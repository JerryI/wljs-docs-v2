# StabilityMarginsStyle

`StabilityMarginsStyle` is an option to frequency response plots such as `BodePlot`, `NyquistPlot`, and `NicholsPlot` that specifies the styles in which the gain and phase margins are to be drawn.

## Examples

```wolfram
BodePlot[sys, StabilityMarginsStyle -> {Red, Blue}]
```

*Please visit the official [Wolfram Language Reference](https://reference.wolfram.com/language/ref/StabilityMarginsStyle.html) for more details.*
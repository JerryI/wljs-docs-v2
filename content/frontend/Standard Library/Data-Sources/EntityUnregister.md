---
title: EntityUnregister
---

`EntityUnregister["type"]` unregisters all entities in the first entity store that defines entities of the specified type.

`EntityUnregister[store]` unregisters all entities in the specified entity store.

## Examples

```wolfram
EntityUnregister["MyEntityType"]
```

*Please visit the official [Wolfram Language Reference](https://reference.wolfram.com/language/ref/EntityUnregister.html) for more details.*
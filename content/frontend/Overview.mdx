...
slug: /Overview
title: What is WLJS
description: Learn how WLJS Notebook combines Wolfram Language, JavaScript, interactive visualizations, and web publishing in a browser-based notebook environment.
---
...
<WLJSWrapper>
<div className="code-input"/>
<wljs-editor type="Input" display="codemirror">{`chords = makeScale[1, scale["Major"], {#,2+#,4+#}] &/@ {
    1, 1, 5, 5, 6, 6, 4, 4, 1 
};
SoundNote /@ chords // Sound`}</wljs-editor>
</WLJSWrapper>
...
```wolfram
Dynamic@Refresh[Now, UpdateInterval -> 1]
```
...
DynamicModule[{pt = {}},
  EventHandler[
    ...
    "MouseMoved" :> (pt = Append[pt, MousePosition["Graphics"]])
  ]
]
```
...
<br/>
  ...
  >
  Migration Guide for Mathematica users
  </Card> 
...
- __Copy group as text__ - copies a group as plain text (GIT and LLM friendly)
...
<Image className="invertColor" width={400} height={400} src={cellProp}/>
</div>

import {Projector} from 'lucide-react'

<b>Project to a window</b> <span className='inline-flex' ><Projector style={{height:"1rem"}}/></span>> is a particularly useful feature that lets you evaluate a cell in a new window. This is especially helpful when presenting slides or distributing multiple controls / graphics outputs across windows.

<Callout type="note">All output Javascript and HTML cells will be automatically copied to a new window</Callout>

<Callout type="idea">Projecting the same cell will refresh the content of the existing window</Callout>

### Context menu
Extended context menu provides you the following features on selected text

import contextProp from './../context-prop.png'

<div style={{'justifyContent': 'space-between'}}className="text-left align-right items-center w-full flex-start flex flex-wrap">
<div className='text-xs' style={{maxWidth:'350px'}}>

- __Evaluate in place__ - evaluates selected expression and replaces it with the result
- __Iconize__ - compresses selected expression (or sequence) to encoded string or to a file (depending on size) and replaces it with an icon
- __Store in notebook__ - copies an expression to a notebook storage (see [NotebookStore](./Cells-and-Notebook/NotebookStore) symbol)
- __Simplify__ - applies [Simplify](./Algebra-and-Polynomials/Simplify) on selected 
- __Speak__ - speaks any selected text using a voice synthesizer 
- __Highlight__ - highlights WL expression with yellow
- __Hidden comment__ - turns selection into a comment in WL
- __Copy__


</div>

<Image className="invertColor" width={400} height={400} src={contextProp}/>
</div>

<Callout>
Options except <small>Speak</small> and <small>Copy</small> are only valid in Wolfram Language cells. 
</Callout>
...
<LazyAutoplayVideo url={"/math_input_crop.mp4"}/>
...
__Shortcuts and hotkeys__

<Card className="text-xs" title="Shortcuts">
...
- <kbd>Ctrl-Shift-Enter</kbd> or <kbd>⌘-Shift-Enter</kbd>: Project cell to a window
...
### File uploading
- Drag and drop to **a sidebar** to upload a file
- Drag and drop to **Wolfram input cell** to upload and import data
- Drag and drop to **Markdown input cell** to upload and import as image
- Use **command palette upload tool** to import `ASCII`, `CSV`, or `TSV` data
- Drag **from a sidebar** and drop to a desktop any files

### Command Palette
Use <kbd>Ctrl-P</kbd> or <kbd>⌘-P</kbd> to access useful tools like matrix builders, color pickers, and more:

<LazyAutoplayVideo url={"/palette_input_cropped.mp4"}/>
...
<LazyAutoplayVideo url={"/flock-fps-1000-ezgif.com-crop-video.mp4"}/>
</Accordion>
...
<LazyAutoplayVideo url={"/image-stream-852-480-4-ezgif.com-crop-video.mp4"}/>
</Accordion>
...
<LazyAutoplayVideo url={"/fluid-sim-15-15-ezgif.com-crop-video.mp4"}/>
</Accordion>
...
<LazyAutoplayVideo url={"/marching-cubes-24642-tri-ezgif.com-crop-video.mp4"}/>
</Accordion>
...
## Human-, LLM- and GIT-friendly Notebook Format [#format]
The structure is minimal, and easy to read and parse by people and machines, but most importantly, it allows versioning systems correctly identify the differences. Here is an example:
...
All heavy blobs of binary/textual data are stored at the very end in a compressed form. 

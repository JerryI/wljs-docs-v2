# GroupSetwiseStabilizer

`GroupSetwiseStabilizer[group, {p1, …, pn}]` returns the subgroup of `group` for which the images of the points `pi` are still in the list `{p1, …, pn}`.

- `GroupSetwiseStabilizer[group, {p1, …, pn}, f]` returns the setwise stabilizer subgroup under the action given by the function `f`.

## Examples

```wolfram
GroupSetwiseStabilizer[SymmetricGroup[4], {1, 2}]
```

```wolfram
GroupSetwiseStabilizer[AlternatingGroup[5], {1, 2, 3}]
```

*Please visit the official [Wolfram Language Reference](https://reference.wolfram.com/language/ref/GroupSetwiseStabilizer.html) for more details.*
...
description: Migrate Mathematica notebooks and Wolfram Language workflows to WLJS, including file formats, formatting, dynamic behavior, compatibility, and frontend differences.
---
...
## Bypass
If something isn't working or does not look correct, but you *really want it* - apply `MMAView` on the expression. For example

```wolfram
Plot[x, {x,0,1}] // MMAView
```

here `Plot` will be rasterized into an image using Mathematica's renderer (sort of) bypassing WLJS system completely using a parallel kernel. It works on 3D graphics too. Consider `MMAView` as a temporal workaround until the missing features are implemented.
...
`Manipulate`, `Animate`, and `Refresh` use a structural JIT diff to replace full reevaluation with granular frontend updates. Before migrating dynamic graphics, read the [JIT optimizer limitations and recommendations](./../GUI/Manipulate#jit-transpiler). Unsupported structural changes fall back to complete result updates and may be slower or flicker; this is a performance fallback, not an evaluation error. Small, textual, slowly changing, or non-latency-sensitive results can remain perfectly practical without JIT optimization. Some changing graphics options are deliberately ignored. When full replacement is observably too slow, keep the output structure stable or build the interaction from lower-level primitives such as `Offload`.
</Callout>
...
Quite often in Mathematica's code it is useful to print live result to a separate cell using `Print` or `PrintTemporary`. In WLJS this can be easily replaced with `Refresh`:
...
To display progress bar, see [ProgressIndicator](./../GUI/ProgressIndicator)

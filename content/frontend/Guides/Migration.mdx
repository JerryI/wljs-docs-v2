...
`Manipulate`, `Animate`, and `Refresh` use a structural JIT diff to replace full reevaluation with granular frontend updates. Before migrating dynamic graphics, read the [JIT optimizer limitations and recommendations](./../GUI/Manipulate#jit-transpiler). Unsupported structural changes fall back to complete result updates and may be slower or flicker; some changing graphics options are deliberately ignored. When the output cannot keep a stable structure, build the interaction from lower-level primitives such as `Offload`.
</Callout>

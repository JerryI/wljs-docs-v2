...
description: Handle user interaction in WLJS with event listeners, input controls, graphics events, and Wolfram Language callbacks for responsive notebook interfaces.
...
If you need a custom element, you can create it directly in the notebook using [WLX](./../Cell-types/WLX) or [JavaScript](./../Cell-types/Javascript) cell types. Please see the guides
...
<Cards>
  ...
  >
    A guide on making custom input elements using Javascript, HTML and WLX
  </Card>
</Cards>
...
You can do the same with [Plot](./../Plotting-Functions/Plot) and many others, since they produce a `Graphics` symbol:

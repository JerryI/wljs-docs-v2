	...
	With[{
	  ...
	  EventHandler[controls, {
	    ...
	    "Save graph" -> AsyncFunction[Null, Module[{path, data},
	      ...
	      If[!StringQ[path], Return[Null, Module]];
	      If[ToLowerCase[FileExtension[FileNameTake[path]]] === "pdf",
	        ExportAsync[path, graphics /. {Offload -> Identity, Locator[_] :> Null, Rule[PlotLabel, _] :> Rule[PlotLabel, None]}] // Await;
	      ,
	        data = RasterizeAsync[graphics /. {Offload -> Identity, Locator[_] :> Null, Rule[PlotLabel, _] :> Rule[PlotLabel, None]}, "ExposureTime"->4] // Await;
	        ExportAsync[path, data] // Await;
	      ];
	      ClearAll[path, data];
	      Beep[];
...
<Callout>
    Note, that during export or rasterization we apply rules `Offload -> Identity, Locator[_] :> Null, Rule[PlotLabel, _] :> Rule[PlotLabel, None]` to remove labels, locators and dynamic coupling (changed to static)
</Callout>
